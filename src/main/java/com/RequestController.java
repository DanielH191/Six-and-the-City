...
public class RequestController extends HelpingMethods {
	...
	public String showFuckingOpenAIrespone() { // @RequestParam String text
		// Assuming textSummarizer.getDataFromText(text) retrieves data based on the
		// provided text
		...
		String[] parts = content.split(content.contains("; ") ? "; " : "\n");

		// Extract location and occurrence from parts if splitting produced the expected
		// number of parts
		String location = "";
		String occurrence = "";
	...
	public Incident getIncidentFromRecording(String audioPath) {
		JsonNode reply = voice2TextService.transcribeAudioFile(audioPath);
		JsonNode choicesNode = reply.get("choices").get(0);
		JsonNode messageNode = choicesNode.get("message");
		String content = messageNode.get("content").asText();
		...
		return new Incident("", "", location, Const.city, Const.postalCode, getDateNow("HH:mm"), Const.durationOfCall,
				occurrence, Const.phoneNumber);
	}
